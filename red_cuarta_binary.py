...
def dividirTestAndTrain(lista,size_tr):
    ...
    for i in range(size_lista):
        ...
        if i < size_lista * size_tr:
            shutil.copy2(archivo,path + '/trainBinA', follow_symlinks=True)
            lista.remove(archivo)
        else:
            shutil.copy2(archivo,path + '/testBinA', follow_symlinks=True)
            lista.remove(archivo)
...
model = Sequential()
model.add(Conv1D(500, 2, activation="relu", input_shape=(1792,1)))
...
print(classification_report(validation_y, y_prediction))
